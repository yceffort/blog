...
안녕하세요. 카카오페이증권 프론트엔드 개발자 김용찬입니다. 외부 SDK의 웹 전송용 번들을 줄이면서, 브라우저에서 계속 동작해야 할 연결까지 다시 살펴본 이야기를 하겠습니다.
-->
...
이 SDK는 이벤트 수집과 A/B 테스트 평가, 메시지 UI를 한 패키지에서 제공했습니다. 홈에서 필요한 것은 화면 진입과 사용자 행동을 수집하는 경로였습니다. 그런데 그 기능을 쓰려고 SDK를 초기화하면 평가와 UI를 조립하는 코드까지 연결됐습니다.
...
_벤더와 패키지, 내부 이름은 가명입니다. 브라우저 API와 기능의 역할, 측정값은 그대로 설명합니다_
...
상수만 가져온 번들은 raw 약 375 KiB, gzip 약 91 KiB였습니다. SDK를 실제로 초기화하는 번들과 비교해도 raw 기준 98%가 남았습니다.
...
둘째는 실제 실행 경로에서 참조하는 경우입니다. 초기화 함수가 평가기와 UI 생성자를 가져온다면, 그 코드는 초기화에서 도달할 수 있습니다. 이때는 어떤 경로가 그 기능을 끌어오는지 봐야 합니다.
...
| ---------------- | --------- | ------------ | ----------- |
| 상수 하나 import | 91.4 KiB  | 24.2 KiB     | **약 −74%** |
| 실제 초기화      | 94.6 KiB  | 93.4 KiB     | **약 −1%**  |

**홈에서 쓰는 초기화 경로의 참조를 바꿔야 했습니다**

_초기 분석 스냅샷의 실험. PURE를 붙인 변경의 동작 안전성은 별도로 검증해야 합니다_
...
따라서 다음 작업은 초기화 경로를 따라가며 전송에 필요한 부분을 따로 조립하는 것이었습니다. 또 PURE는 개발자가 번들러에 주는 정보입니다. 코드가 빠졌다는 것과 그 변경이 안전하다는 것은 따로 확인해야 합니다.
-->
...
이제 전송에 필요한 코드만 따로 조립해 보겠습니다. 먼저 원본 소스를 확보하고, 초기화에서 평가기로 이어지는 참조를 따라갔습니다.
-->
...
_소스맵과 선언을 합쳐 수백 개 파일을 복원했습니다. sourcesContent가 없으면 코드 전문을 추출할 수 없습니다_
...
소스맵에 위치 정보만 있다고 원본 코드가 복원되는 것은 아닙니다. 이 작업에서는 코드 전문이 포함되어 있었기 때문에 가능했습니다. 구체적인 추출 순서는 부록에 남겨 두었습니다.
-->
...
전송용에서는 `evaluation: false`를 넘겼지만,<br>이 빌드에서는 **분기가 접히지 않아 평가기 참조가 남았습니다**
...
실행할 때 생성하지 않는다는 목적은 달성했지만, 이번 빌드에서는 이 분기가 항상 false라고 접히지 않았습니다. 공통 코어 파일에 평가기를 import하고 생성하는 경로가 그대로 남았기 때문입니다.

모든 조건 분기가 트리셰이킹을 막는다는 뜻은 아닙니다. 여기서는 이 코드와 이 빌드 조건에서 제거되지 않았다는 관측이 중요합니다. 그래서 플래그 값에 기대는 대신 전송 엔트리에서 평가기를 향하는 import 자체를 없애는 쪽으로 바꿨습니다.
-->
...
_이름과 코드는 구조를 설명하기 위한 요약입니다. 타입 참조와 런타임 import를 구분했습니다_
...
_코드와 import 관계는 핵심 경로만 요약했습니다_
...
_[Rollup 공식 문서](https://rollupjs.org/configuration-options/#output-preservemodules) / 트리셰이킹은 계속 적용됩니다. 설정만으로 크기 감소를 보장하지 않으므로 소비자 번들에서 다시 측정했습니다_
...
이 설정 하나가 트리셰이킹의 정답이라는 뜻은 아닙니다. 앞에서 실제 참조를 끊은 구조와 함께 적용한 선택입니다. 마지막에는 dist 파일을 직접 읽는 대신 패키지 이름으로 import해서, 소비자가 받는 번들을 다시 측정했습니다.
-->
...
_중간 모듈은 생략했습니다. 파일 목록만으로 엔트리의 포함 범위를 판단할 수 없었습니다_
...
**재구성 결과: 웹 전송용 gzip 94.9 → 29.5 KiB, 약 69% 감소**

_이후 크기 표는 2026-09-05 웹용 측정본을 사용합니다_
...
그런데 필요한 함수를 남겨 둔 것만으로는 충분하지 않았습니다. 엔트리를 나누는 과정에서 그 함수를 실행시키는 연결을 놓쳤습니다.
-->
...
> **공개 API 검사만으로는 실행 연결의 누락을 잡지 못했습니다**
...
엔트리를 나누면서 이 동작을 연결하는 코드가 전체 엔트리에만 남았습니다. 다른 엔트리에는 함수가 있어도 트래커와 리스너, 라이프사이클을 여는 연결이 빠져 있었습니다.
...
_beacon 지원 환경의 실행 경로를 요약했습니다. 전송 요청의 관찰과 서버 수집 확인은 구분합니다_
...
_jsdom에서 XHR와 sendBeacon 요청 본문을 관찰했습니다. 서버에 저장되었다는 검사는 별도입니다_
...
그리고 해당 리스너 등록을 일부러 지웠습니다. 이때 테스트가 실패해야 이 연결을 정말 보고 있다고 말할 수 있습니다. 다만 여기서 확인한 것은 jsdom에서 관찰한 전송 요청입니다. 실제 서버 수집은 뒤에서 별도로 확인했습니다.
-->
...
## 같은 동작과 의도적으로 바꾼 동작을 나눴습니다

| 구분        | 확인할 대상                              | 검증 방법                           |
| ----------- | ---------------------------------------- | ----------------------------------- |
| 유지할 동작 | 이벤트의 비교 대상 필드, 평가와 URL 분기 | 원본과 같은 입력으로 대조           |
| 자동 동작   | 세션과 페이지뷰, 이탈 전송               | 엔트리별 실행과 요청 검사           |
| 의도적 차이 | 웹 전송용의 초기화 방식과 일부 필드      | 양쪽 값을 각각 단언, 수집 영향 확인 |

**전송용뿐 아니라 함께 재구성한 다른 웹 엔트리도 확인했습니다**

_다음 URL 분기 사례는 리다이렉트 기능을 포함한 엔트리의 검증에서 나왔습니다_

<!--
발표 40초 / 13:30~14:10

다음 사례로 넘어가기 전에 무엇을 같게 만들려고 했는지 짚겠습니다. 전송용뿐 아니라 평가와 URL 분기 같은 다른 웹 엔트리도 함께 재구성했습니다. 유지할 반환값과 이벤트 필드는 원본과 대조했고, 자동 동작은 엔트리별로 실행했습니다.

반면 전송용의 초기화 방식과 일부 필드에는 의도적인 차이가 있었습니다. 그 차이는 비교에서 조용히 숨기지 않고 양쪽 값을 각각 단언하고 수집과 집계에 미치는 영향을 따로 확인했습니다.

지금부터 볼 테스트 격리 문제는 URL에 따라 리다이렉트하는 엔트리를 검증하다가 발견한 것입니다.
-->
...
_난수와 시각, URL 유래 값은 비교 범위와 처리 방식을 정했습니다. 의도적 차이는 별도 단언합니다_

<!--
발표 50초 / 14:10~15:00

기대값을 제가 직접 적는 것만으로는 원본과 같은지 알기 어려웠습니다. 원본을 잘못 읽으면 그 오해가 기대값에도 들어갈 수 있기 때문입니다. 그래서 배포된 원본 UMD를 별도 jsdom에서 실행하고, 재구성본은 테스트의 jsdom에서 같은 설정과 입력으로 실행했습니다.
...
그래서 다음 테스트가 URL을 설정하려고 pushState를 호출할 때 앞 테스트의 인스턴스도 반응했습니다. 모듈을 다시 읽고 스토리지를 비우는 것만으로는 이 함수 참조가 원래대로 돌아오지 않았습니다.
-->
...
| ---------------------- | ------------- | ------------ | --------------------- |
| **`./send` 웹 전송용** | **100.0 KiB** | **29.5 KiB** | **약 −69%**           |
| `./evaluate` 평가 포함 | 173.2 KiB     | 46.5 KiB     | 약 −51%               |
| `.` 전체 엔트리        | 264.3 KiB     | 66.0 KiB     | 약 −30%               |
| 벤더 원본              | 383.4 KiB     | 94.9 KiB     | 기준                  |

**엔트리 분리와 빌드, 폴리필, 의존성 변경을 함께 반영한 결과입니다**

_2026-09-05 웹용 측정본 / 패키지 이름으로 import / esbuild 0.24.2 / browser / esm / es2017 / gzip 9<br>네이티브 연동은 소비자 측 범위. 초기 진단과 별도 측정이며, raw는 minify 후 압축 전 크기입니다 (1 KiB = 1,024 B)_

<!--
발표 45초 / 18:50~19:35

웹용 측정본의 결과입니다. 벤더 기준은 gzip 약 94.9 KiB이고 웹 전송용은 약 29.5 KiB입니다. 약 69% 줄었습니다. 평가를 포함한 엔트리는 약 51%, 전체 엔트리도 약 30% 줄었습니다.

엔트리 분리 하나의 효과는 아닙니다. 모듈 구조와 빌드 타깃, 폴리필과 의존성 변경을 함께 적용한 결과입니다. 또 전송용은 앞에서 정한 웹 수집 범위의 산출물이며, 웹뷰 연동은 필요한 서비스에서 별도로 구성합니다.

도입의 PURE 실험과는 버전과 측정 시점이 다른 결과입니다. 초기 진단은 원인을 가르는 실험이었고, 이 표는 9월 5일 웹용 산출물을 소비자 방식으로 가져와 측정한 값입니다.
-->
...
_모바일 에뮬레이션 / CPU 4배 감속 / 콜드 로드 / 개선 전 4회와 후 5회의 중앙값 비교. SDK 단독 기여분은 분리 측정하지 않았습니다_

**별도 JavaScript 실험:** 같은 5MiB, CPU 4배 감속에서 메인 스레드 작업은<br>미호출 함수 **81.5ms** / 최상위 초기화 **915.3ms**

_[실험 출처와 조건](https://yceffort.kr/2026/09/unused-javascript-cost) / 각 15회 중앙값, VM 내부 통신. SDK의 절감 시간으로 환산한 값은 아닙니다_
...
홈에서는 이 SDK 재구성과 다른 번들 정리, 렌더링 개선을 함께 진행했습니다. 같은 조건에서 개선 전 네 번과 후 다섯 번의 중앙값을 비교했습니다. 첫 화면이 표시되는 FCP는 44.3%, 초기 5초의 CPU 지표는 30% 줄었습니다. 위 표는 홈 전체의 합산 결과이고 SDK 하나의 기여분을 분리한 수치는 아닙니다.

그렇다면 바이트 감소를 어떤 비용과 연결해서 봐야 할까요? 별도로 수행한 JavaScript 실험이 있습니다. CPU를 네 배 늦춘 조건에서 같은 5MiB라도, 함수를 호출하지 않으면 메인 스레드 작업이 81.5ms였고 최상위에서 초기화하면 915.3ms였습니다. 준비 시간 전체가 아니라 메인 스레드 작업 시간의 비교입니다.

이 숫자를 SDK의 65 KiB 감소에 비례해서 적용하지는 않습니다. 실험이 알려주는 것은 어디를 봐야 하는가입니다. 느린 회선에서는 전송할 바이트를, CPU가 느린 환경에서는 로딩 중 컴파일하고 실행하는 초기화 코드를 함께 봐야 합니다. 그래서 번들 크기를 확인한 다음, 홈의 화면 표시와 초기 CPU 비용도 확인했습니다.
-->
...
> **웹 수집 경로를 확인한 뒤 운영에 반영했습니다**

_샌드박스는 수집을 분리해 둔 환경입니다. 확인한 입력과 환경의 근거이며 모든 동작의 동등성을 뜻하지는 않습니다_

<!--
발표 45초 / 20:45~21:30

마지막으로 코드 밖에서 데이터가 실제로 쌓이는 곳까지 확인했습니다. 수집을 분리해 둔 샌드박스에서 Playwright로 화면을 조작해 이벤트를 발생시키고, 대시보드에 기록되는지 봤습니다. 같은 환경의 기존 수치가 흔들리지 않는지도 관찰했습니다.

jsdom에서 전송 요청을 잡았다는 것만으로 서버에 저장됐다고 말할 수는 없으므로 두 확인을 나눴습니다. 의도적으로 달라진 필드도 수집과 집계의 영향을 따로 봤습니다.

이 과정을 거쳐 웹 수집 경로를 운영에 반영했습니다. 다만 확인한 입력과 실행 환경의 근거라는 범위는 남습니다. 모든 기능과 모든 환경에서 완전히 같다는 주장은 하지 않습니다.
-->
...
마지막은 엔트리의 계약입니다. 웹 전송용이 맡는 범위와 소비자가 연결할 범위를 정하고, 유지할 동작과 의도적으로 바꾼 동작을 나눠 확인했습니다. 이 세 가지를 함께 보고 웹 전송용 번들의 gzip 크기를 약 69% 줄였습니다. 다른 개선을 함께 적용한 홈에서는 첫 화면 표시와 초기 CPU 비용도 확인했습니다.
-->
...
코드가 왜 따라오는지와, 남긴 코드가 언제 실행되는지를 함께 보시면 좋겠습니다. 지금까지 카카오페이증권 김용찬이었습니다. 감사합니다.
-->
