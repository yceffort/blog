---
title: 'OG 스크래핑 서버를 Node.js로 짓는다면 3부: 에러율과 지연을 낮추기'
marp: true
paginate: true
theme: midnight
tags:
  ...
  - deep-dive
date: 2026-08-18
description: 'User-Agent와 인코딩으로 에러율을 낮추고, 가져온 값을 안전하게 그리고, 캐시 스탬피드와 히트율 역산으로 지연 목표를 세우는 법'
published: true
---
...
- 1부: "에러율 10%"는 증상이고, 성질이 다른 실패 다섯 종류가 뭉쳐 있었다
- 1부: 이 워크로드는 I/O 바운드에 저 TPS라 런타임 후보가 좁혀졌다
- 2부: 사용자가 준 URL을 여는 일은 SSRF의 교과서적 조건이고, 우회는 여섯 가지였다

여기까지는 **뚫리지 않는 법**이었다. 이 부는 **되게 만드는 법**이다.

> 1·2부를 읽지 않아도 된다. 필요한 전제는 두 줄이다.
> **우리 서버는 남의 URL을 대신 연다. 그리고 그 상대는 우리 편이 아니다.**

---

## 이 부가 답하려는 것

1. User-Agent 하나가 왜 에러율을 가르는가
2. 한국어 사이트의 인코딩은 왜 아직도 깨지는가 (여기서 Node가 불리하다)
3. **가져온 값을 그대로 화면에 그려도 되는가**
4. 캐시는 무엇을 고치고, **무엇을 못 고치는가**
5. 캐시 스탬피드는 왜 특히 나쁘고, 어떻게 막는가
6. "P95 1초 미만"이라는 목표를 **히트율에서 역산하는** 법

> 3부 목차: 4장 스크래핑 실무 · 5장 캐싱 · 6장 목표 수치 세우기 · 7장 정리
...
- 실패한 도메인에 **재시도를 반복하지 않는다** (5장에서 다룰 negative cache가 여기서도 쓰인다)
- 조직 차원의 정책으로 문서화한다
...
> 1부 2장에서 "Node가 불리한 지점"이라고 했던 게 이 자리다.
...
> 5장에서 "커버리지를 따로 재라"고 말할 텐데, 그 이유가 이것이다.
> **성공 응답 중에도 실패가 숨어 있다.**
...
트리를 만드는 `parse5`(1부 2장) 대신, 조각을 그때그때 읽어 멈출 수 있는 **htmlparser2**(스트리밍 파서)를 쓴다.
...
— 5장의 "커버리지"를 조금 깎는다. 커버리지가 더 중요하면 상한(512KB)까지 계속 읽는다.
...
2부는 **우리 서버가 나가는** 방향을 지켰다. 남이 준 URL로 아무 데나 요청하지 않도록.
...
> 원칙은 3장과 같다. **막는 자리는 검증하는 자리가 아니라 쓰는 자리다.**
...
**3. 그렇다고 우리 서버가 대신 받아오면 2부가 그대로 재현된다**
이미지 프록시는 **사용자 입력 URL을 서버가 여는 일**이다. 3장의 검증을 통째로 다시 적용해야 한다.
...
- 값이 빈 문자열인 경우와 태그 자체가 없는 경우를 구분한다. 5장의 negative caching에서 둘의 수명이 다르다
...
- [ ] `og:image`를 프록시할 것인지 정하고, 프록시한다면 3장 검증을 그대로 적용한다
- [ ] 길이 상한과 중복 태그 규칙을 정한다

> 2부에서 41장을 들여 "남이 준 URL을 믿지 말라"고 했다.
...
403·로그인 벽·인코딩처럼 그 URL이면 늘 실패하는 것은, negative caching으로 캐시해도 여전히 실패 응답이라 에러율이 내려가지 않는다(1부 1장 표 참조).
...
②를 올리려면 UA 조정, 리다이렉트 처리, 인코딩 대응처럼 **4장의 것들**을 해야 한다.
...
> 1부 1장의 평시 규모(0.06 TPS)로는 이런 순간이 잘 안 온다. 문제는 평균이 아니라
...
> 2부 3장의 "주소 정규화를 직접 하지 말라"와 헷갈리지 말 것.
...
**실패의 성질이 다르니 수명도 달라야 한다.** 1부 1장에서 실패를 원인별로 분류하라고 한 것이 여기서 쓰인다.
