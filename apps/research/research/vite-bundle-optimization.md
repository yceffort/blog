...
**같은 패키지인데 JS에서 막힌 게 CSS에서는 통한다.** resolve 경로가 다르기 때문이다.

---

## 대신 구조적 리스크를 안는다

`dist/<component>/index.css`는 **`exports`에 선언되지 않은 경로**다. 패키지 입장에서는 사적 구현이다.

- 업스트림이 dist 레이아웃을 바꾸면 빌드가 깨진다
- 컴포넌트를 새로 쓰기 시작하면 `@use` 추가를 잊는다 → 스타일 누락
- 버전업 때마다 확인이 필요하다
...
```
축 2 적용 전 : 디자인시스템 청크 89.99 kB gz   ← external로 빼면 −89
축 2 적용 후 : 디자인시스템 청크 31.87 kB gz   ← external로 빼면 −31.81
```
...
**축 1~3은 앱 단독 설정 변경이고, 축 4는 구조 변경이다.** 싸고 확실한 것부터.
