...
1~4부 끝에 **중간 점검 한 문제**, 마지막에 **종합 퀴즈 6문제**가 있다.
...
서울에서 미국 버지니아까지 대권 거리는 약 11,000km다. 광섬유 안에서 빛은 약 200,000km/s로 움직인다.
...
HTTPS로 첫 바이트를 받기까지는 왕복이 한 번이 아니다.

```text
TCP 핸드셰이크   1 RTT
TLS 핸드셰이크   1 RTT (TLS 1.3)
HTTP 요청/응답   1 RTT
─────────────────────
첫 바이트까지    3 RTT ≈ 550ms
```

이게 HTML 한 장이고, 그 HTML이 참조하는 JS와 CSS가 다시 이 비용을 낸다.
...
- **DNS 기반** : 요청한 리졸버의 위치를 보고 DNS 응답에 지역별로 다른 IP를 돌려준다
- **Anycast** : 여러 엣지가 **같은 IP**를 광고하고, 인터넷 라우팅(BGP)이 가장 가까운 곳으로 보낸다. Cloudflare, Vercel이 이 방식이다
...
- **배포(distribution)** 하나가 CDN 한 벌이다. `d1234.cloudfront.net` 도메인을 받고 여기에 내 도메인을 붙인다
- **오리진** : 사본의 출처. 정적 파일은 S3 버킷, 동적 응답은 ALB나 EC2
- **동작(behavior)** : 경로 패턴별 규칙. `/static/*`는 S3로, `/api/*`는 ALB로 보내고 각각 다른 캐시 정책을 붙인다
- 엣지 뒤에 **리전 엣지 캐시**가 한 단계 더 있어, 엣지에서 빠진 사본도 오리진까지 안 가고 받는 경우가 많다
...
솔직한 답부터: **거리는 국내에서 약한 논거다.** 서울에서 서울 리전 S3까지 TCP 연결이 10ms 안팎이다(실측). CDN은 멀리 있는 사용자용 장치가 아니라 **오리진 앞에 두는 공유 캐시**이고, 그 역할은 국내에서도 그대로 필요하다.

1. **오리진 부하 분리** : 사용자 만 명이 와도 정적 파일은 오리진이 한 번만 응답한다. 트래픽이 튈 때 서버 대신 엣지가 흡수한다. 앞서 잰 MISS 372ms 대 HIT 50ms도 거리보다 "오리진을 실행했느냐"의 차이다
2. **비용** : S3에서 CloudFront로 가는 전송은 무료이고, CloudFront 발신이 EC2나 ALB 발신보다 GB당 싸다
3. **가용성** : 오리진이 죽어도 `stale-if-error`로 옛 사본을 내보내고, DDoS 완화와 WAF가 오리진 앞에서 막는다
4. **국내 사용자만 있는 서비스는 없다** : 해외 체류자, 외국인, Googlebot 같은 크롤러는 해외에서 온다
...
캐시는 응답을 받을 때 두 값을 비교한다.

```text
freshness_lifetime = max-age (CDN이면 s-maxage 우선)
current_age        = Age 헤더 값 + 내가 받은 뒤 흐른 시간

current_age < freshness_lifetime  →  fresh, 서버에 묻지 않고 쓴다
그 외                             →  stale, 재검증하거나 새로 받는다
```

`Age`는 **앞단 캐시가 이 사본을 얼마나 들고 있었는지**다. CDN이 3일 묵힌 사본을 주면서 `max-age=31536000`이라고 하면, 브라우저는 3일을 빼고 남은 기간만 신선하다고 본다.
...
## 브라우저가 보는 헤더와 CDN이 쓴 규칙은 다를 수 있다
...
## CloudFront는 TTL을 이렇게 정한다

캐시 정책(cache policy)의 **Min, Default, Max TTL** 세 값이 오리진 헤더와 이렇게 합쳐진다.

| 오리진이 보낸 것                  | CloudFront가 캐시하는 시간                                     |
| --------------------------------- | -------------------------------------------------------------- |
| `max-age=N` (또는 `s-maxage=N`)   | N을 Min과 Max 사이로 자른 값. `s-maxage`가 있으면 그쪽         |
| 캐시 헤더 없음                    | Default TTL. 관리형 정책은 **24시간**                          |
| `no-cache`, `no-store`, `private` | Min TTL이 0이면 존중. **Min TTL > 0이면 그 시간만큼 캐시한다** |

- **CachingOptimized** (정적 자산용): Min 1초, Default 24시간, Max 1년. 쿼리스트링, 쿠키, 헤더를 전부 키에서 뺀다. Min이 1초라 `no-store`를 붙여도 1초는 캐시된다(문서의 경고문)
- **CachingDisabled** (동적 응답용): 세 값 모두 0초
- **UseOriginCacheControlHeaders**: Min 0, Default 0, Max 1년. 오리진 헤더를 그대로 따를 때
...
- 브라우저가 보는 헤더는 CDN이 고쳐 보낸 최종본일 수 있다. `Age`와 HIT/MISS 헤더로 역추적한다
...
## 함정 4 : 공용 CDN의 라이브러리는 더 이상 공짜가 아니다
