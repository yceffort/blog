...
| ------------------- | ---------: | --------------------------------------------------------- | -------------: |
| 기존 설정           |    1,448ms | 5,636ms (문단), 1,552ms (제목, 계측 이상), 5,636ms (문단) |       168.3KiB |
| 수식 글꼴 즉시 전달 |    1,456ms | 2,804ms (배너), 2,792ms (배너), 1,524ms (제목, 계측 이상) |       168.3KiB |
| 수식 글꼴 요청 지연 |    1,448ms | 1,528ms (제목, 계측 이상), 6,020ms (문단), 6,028ms (문단) |       168.3KiB |
| 화면 전환 해제      |    1,456ms | 5,640ms (문단), 5,660ms (문단), 5,640ms (문단)            |       168.3KiB |
| 공통 웹 글꼴 제외   |      688ms | 2,312ms (배너), 2,316ms (배너), 2,368ms (배너)            |           0KiB |

수식 글꼴을 즉시 전달하면 늦은 문단 후보가 사라졌고, 요청을 늦춘 조건에서 문단이 최종 후보인 실행은 약 6초가 됐다. 화면 전환을 끈 세 번도 약 5.6초였으므로 화면 전환만으로 지연을 설명하는 가설은 철회했다.

표에서 "계측 이상"으로 표시한 세 실행은 조건의 효과로 읽으면 안 된다. 제목의 LCP `size`가 23,400으로 기록됐는데 뷰포트 폭 390에 60을 곱한 값이고, 같은 제목이 공통 글꼴을 유지한 나머지 아홉 번에서는 5,226, 시스템 글꼴을 쓴 세 번에서는 5,092다. 콜백에서 읽은 사각형은 열다섯 번 모두 326 곱하기 67.1875로 같으므로, 중간 레이아웃의 페인트가 잡힌 것이다. LCP는 직전 최대보다 큰 후보만 새로 기록하고 문단은 20,672, 배너는 10,335이므로, 23,400이 박힌 뒤로는 아무것도 기록되지 못한다. 화면 전환을 실행한 열두 번 중 이 세 번만 후보가 세 개에서 끝났다(공통 글꼴을 제외한 세 번도 후보가 세 개지만 배너에서 정상 종료한 경우다). 세 번 모두 화면 전환이 실행된 열두 번 안에서 나왔고 해제한 세 번에서는 없었지만 원인은 확인하지 못했다. 이 셋을 빼면 기존 설정, 즉시 전달, 요청 지연 조건은 두 번씩 남고 방향은 같다. 공통 글꼴을 제외하면 FCP가 줄었지만 LCP 대상도 배너로 바뀌었고, 수식 글꼴 응답 완료는 약 5.55초에서 약 4.8초로 앞당겨졌을 뿐 여전히 늦었다.

같은 기준을 첫 48회에도 적용해야 한다. 코드 글 변경 전 첫 방문 4회 중 1회는 제목이 `size` 34,238(뷰포트 폭 390 × 87.8)로 2,256ms에 기록된 뒤 더 큰 후보가 없었고, 나머지 3회는 배너가 3,236ms에서 3,252ms 사이였다. 이 1회도 같은 계측 이상이므로 코드 글 변경 전 LCP는 4회 중앙값 3,242ms가 아니라 나머지 3회의 3,248ms로 읽는다. 48회 원자료는 마지막 후보만 남겨 이력을 감사할 수 없고, 이 판단은 `size`와 시간만으로 내린 것이다. 현재 `compare-performance.mjs`는 후보 이력(`lcpEntries`)과 콜백 시점 사각형을 함께 저장한다.
...
로딩 중 화면 확인 두 번도 따로 실행해 [lcp-visual-check.json](lcp-visual-check.json)에 보존했다. 약 2초에 제목과 첫 문단의 시작이 이미 보였고, 수식 글꼴은 로딩 중이었다. 최종 LCP는 5,640ms와 5,632ms였다. `LCP_SNAPSHOTS=1`은 `document.fonts.ready`를 기다리지 않는 CDP `Page.captureScreenshot`으로 로딩 중 화면을 저장한다. 화면을 수집하는 부가 작업이 있으므로 이 실행을 위 15회와 합산하지 않았다. 본문에 실은 첫 화면의 파일 해시도 기록했다. 다만 `lcp-visual-check.json`의 `publishedScreenshot.sha256`(`d1057773…`, 220,311바이트)은 캡처 원본이고, 저장소의 `public/2026/09/images/blog-performance/lcp-before-font-loaded.png`는 이미지 최적화 액션(69c82225)이 다시 압축한 47,394바이트 파일(SHA-256 `2d527407d4ba72bf0fb794c5fe05d223798e2fae37a9f11d6384149097d1248b`)이다. 화면 내용은 같고 바이트는 다르다.
