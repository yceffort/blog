...
그러니까 이 편 전체가 그 이야기였다. 반대쪽인 되게 만드는 법, 그러니까 User-Agent 하나가 왜 에러율을 가르는지, 한국어 사이트의 인코딩은 왜 아직도 깨지는지(여기서는 Node가 불리하다), 가져온 값을 그대로 화면에 그려도 되는지, 그리고 "P95 1초 미만" 같은 목표를 캐시 히트율에서 역산하는 방법은 [1편](/2026/08/og-scraping-server-1)에서 다뤘다. 두 편이 이 시리즈의 전부다.
