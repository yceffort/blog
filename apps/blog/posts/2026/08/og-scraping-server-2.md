...
여기서 얻는 것은 **이름은 목적지에 대해 아무것도 말해주지 않는다**는 점이다. 봐야 하는 건 그 이름이 해석된 IP다.

### 우회 3: DNS rebinding

그러면 이름 대신 해석된 IP를 검사하면 되는가. 그것도 뚫린다. 여섯 중 가장 교묘하고 가장 많이 놓치는 것이라고 생각한다.
...
여기서 얻는 것은 검사한 것과 실제로 쓰는 것이 같아야 한다는 점이다. 이름을 검사하는 대신 **주소를 검사한 뒤 그 주소를 그대로 써야 한다.**
...
여기서 얻는 것은 **파싱은 한 번만 하고, 파싱한 그 객체를 그대로 요청에 넘긴다**는 점이다.
...
| 검증한 값과 요청하는 값이 다름    | 한 번 파싱한 객체만 쓰고 **스킴·포트도 잠근다** (원리 4)             |
