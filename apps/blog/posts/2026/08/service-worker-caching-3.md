...
1편이 도입 조건 셋을 꼽았을 때 그것은 전부 일반론이었다. 세 편을 재고 나니 그중 하나에는 검사법을 붙일 수 있게 됐다. HTTP 캐시로 표현할 수 없는 전략이 필요한 경우인데, 자기 사이트가 거기 해당하는지는 응답 헤더를 열어 보면 알 수 있다. 개발자 도구에서 페이지마다 반복해서 나가는 요청을 골라 응답의 `cache-control`에 `no-store`나 `no-cache`가 붙어 있는지 본다. `no-store`면 브라우저 캐시는 그 응답을 저장하지도 않고, `no-cache`나 `max-age=0, must-revalidate`면 저장은 하되 쓸 때마다 서버에 다시 물어야 해서 왕복이 남는다. 어느 쪽이든 그런 요청이 한 페이지에 수십 건씩 나간다면 이 블로그가 만난 것과 같은 상황이다. App Router의 RSC 프리페치가 정확히 그랬다. 프레임워크가 자체 프리페치를 많이 쏘는 스택일수록 이 조항에 해당할 가능성이 높다.
...
| 브라우저 캐시가 못 쓰는 요청이 페이지마다 수십 건씩 나간다 | 쓴다. 이 블로그는 웜 LCP가 460ms에서 200ms로 줄었다                                                          |
