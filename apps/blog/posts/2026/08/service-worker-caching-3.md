...
먼저 내 장치를 의심했다. 이 설계는 워커 조건만 홈에서 `controllerchange`를 기다리느라 3.5초를 더 머문다. 두 조건의 체류 시간을 맞추고 스무 쌍을 다시 돌려도 460 대 200이었고, 조건 실행 순서를 뒤집어 열두 쌍을 더 돌려도 524 대 200으로 방향이 그대로였다. 장치 탓은 아니었다.
...
31건이라는 수에도 단서를 달아야 한다. 헤드리스 기본 뷰포트(1280x720)에서 뷰포트에 들어온 링크 수다. 화면이 작은 실사용자 기기에서는 처음에 더 적게 나가고 스크롤하면서 늘어난다. 크기를 이 숫자 그대로 가져갈 수는 없고 방향만 가져갈 수 있다.
...
그러면 성능에는 도움이 되는가. 이 블로그에서는 된다. 다만 2편이 도움이라고 읽은 자리가 아니었다. 정적 자산을 Cache Storage에 넣어 재방문 FCP를 줄인 것이 아니라, HTTP 캐시가 재검증 없이는 내주지 못하는 RSC 프리페치를 워커가 대신 내준 것이다. 1편이 서비스 워커를 쓸 이유로 꼽은 셋 가운데 세 번째, "HTTP 캐시로는 표현할 수 없는 전략이 필요할 때"에 정확히 해당한다. 1편은 그 조항을 일반론으로 적었고 이 블로그가 거기 해당하는지는 몰랐는데, 이제는 안다.

같은 자리에서 1편의 다른 문장 하나는 좁혀야 한다. "목표가 재방문 성능 하나라면 이 레이어는 답이 아닐 가능성이 높다"고 쓰면서 근거로 "HTTP 캐시가 이미 해주는 일을 코드로 다시 만드는 셈"이라고 했다. 앞부분은 이 블로그에서 확인됐다. 정적 자산에 관한 한 Cache Storage는 HTTP 캐시보다 빠르지 않았다. 흔들리는 것은 "이미 해주는 일"이라는 전제 쪽이다. App Router의 RSC 프리페치처럼 HTTP 캐시가 원래 못 해주는 일이 섞여 있으면, 재방문 성능이라는 같은 목표에서도 답이 달라진다.

이 결론이 서는 조건도 그어 둬야 한다. 이 블로그는 CDN이 이미 빠르고(엣지 HIT에서 최종 헤더까지 46ms다), 정적 자산에 `immutable` 캐시 헤더가 제대로 박혀 있고, LCP 요소가 텍스트다. 세 조건이 다르면 답도 다르다. 특히 원본 서버가 느리거나 CDN이 없는 사이트라면 Cache Storage가 지우는 왕복의 크기 자체가 달라져서 정적 자산 쪽에서도 이득이 날 수 있는데, 이 시리즈는 그런 환경을 한 번도 재지 않았다. 여기서 얻은 것은 "서비스 워커는 성능에 쓸모없다"가 아니라 "이 조건에서는 이득의 자리가 여기였다"이다.

마지막으로 하나 더 밝혀 둔다. 이 시리즈는 이 워커가 값어치를 하는 축을 처음부터 한 번도 재지 않았다. 오프라인이다. 성능 지표는 네트워크가 있을 때 얼마나 빠른가를 재는 자이고, 이 기능을 넣은 이유는 네트워크가 없을 때도 열리게 하는 것이었다. 세 편에 걸쳐 재고 되돌리고 다시 잰 숫자들이 전부 이 기능의 목적과는 조금 어긋난 자를 대고 있었던 셈이다. 그래도 무의미하지는 않다고 본다. 오프라인이 필요해서 넣는 것이라면 성능은 도입의 이유가 아니라 함께 지불하거나 돌려받는 값이고, 그 값이 어느 쪽으로 얼마인지는 알고 넣는 편이 낫다.
