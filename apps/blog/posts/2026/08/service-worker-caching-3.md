...
1편이 도입 조건 셋을 꼽았을 때 그것은 전부 일반론이었다. 세 편을 재고 나니 그중 하나에는 검사법을 붙일 수 있게 됐다. HTTP 캐시로 표현할 수 없는 전략이 필요한 경우인데, 자기 사이트가 거기 해당하는지는 응답 헤더를 열어 보면 알 수 있다. 개발자 도구에서 페이지마다 반복해서 나가는 요청을 골라 `max-age=0`이나 `must-revalidate`가 붙어 있고 `vary`에 여러 헤더가 걸려 있는지 본다. 그렇다면 브라우저 캐시는 그 요청을 재검증 없이 내주지 못한다는 뜻이고, 그런 요청이 한 페이지에 수십 건씩 나간다면 이 블로그가 만난 것과 같은 상황이다. App Router의 RSC 프리페치가 정확히 그랬다. 프레임워크가 자체 프리페치를 많이 쏘는 스택일수록 이 조항에 해당할 가능성이 높다.

정리하면 이렇다.

| 상황                                               | 이 시리즈의 답                                                                      |
| -------------------------------------------------- | ----------------------------------------------------------------------------------- |
| 오프라인이 실제 요구사항이다                       | 쓴다. 성능은 도입의 이유가 아니라 함께 오는 값이다                                  |
| 페이지마다 재검증이 필요한 요청이 수십 건씩 나간다 | 쓸 값어치가 있다. 이 블로그가 얻은 LCP 이득은 전부 여기서 나왔다                    |
| 네트워크가 불안정한 사용자가 많다                  | 쓰되 network-first에 타임아웃 폴백을 먼저 붙인다. 그것 없이는 lie-fi에서 무력하다   |
| 정적 자산의 재방문 성능만 목표다                   | 쓰지 않는다. 해시와 `immutable`로 이미 끝난 일이고 Cache Storage가 더 빠르지도 않다 |

쓰기로 했다면, 이 시리즈에서 값을 치르고 배운 것이 다섯 있다.

- navigation preload를 켠다. 콜드 기동에서 두 자릿수 ms를 돌려받고, 켜지 않을 이유가 없다.
- 가로챌 필요가 없는 요청은 `respondWith()`를 부르지 않고 통과시킨다. fetch 핸들러는 모든 요청에 비용을 부과한다.
