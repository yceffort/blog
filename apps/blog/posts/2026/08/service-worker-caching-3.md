...
그러면 언제 쓰면 되는가. 1편이 세 조건을 꼽았는데, 이 시리즈를 다 재고 나니 둘은 그대로 두고 하나에는 검사법을 붙일 수 있게 됐다. 오프라인이 요구사항이면 성능은 따질 것도 없이 쓰는 것이다. 네트워크가 불안정한 사용자가 많은 경우도 마찬가지인데, 2편에서 적었듯 지금 구현은 타임아웃 폴백이 없어 lie-fi에서는 무력하니 그 경우라면 network-first에 타임아웃을 붙이는 것이 먼저다. 남은 하나가 어려웠다. HTTP 캐시로 표현할 수 없는 전략이 필요할 때라는 조항은 일반론으로만 읽으면 자기 사이트가 해당하는지 알 길이 없다. 이제는 볼 데를 안다. 개발자 도구에서 페이지마다 반복해서 나가는 요청을 골라 응답 헤더를 열어 보면 된다. `max-age=0`이나 `must-revalidate`가 붙어 있고 `vary`에 여러 헤더가 걸려 있으면 브라우저 캐시가 그 요청을 재검증 없이는 내주지 못한다는 뜻이고, 그런 요청이 한 페이지에 수십 건씩 나간다면 이 블로그가 만난 것과 같은 상황이다. App Router의 RSC 프리페치가 정확히 그랬다. 프레임워크가 자체 프리페치를 많이 쏘는 스택일수록 이 조항에 해당할 가능성이 높다.

거꾸로 목표가 정적 자산의 재방문 성능 하나라면 이 시리즈의 답은 쓰지 말라는 쪽이다. 해시가 박힌 자산에 `immutable`을 붙이는 것으로 이미 끝난 일이고, Cache Storage가 그보다 빠르지 않다는 것은 랩에서도 프로덕션에서도 같았다.
