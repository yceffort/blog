...
| 조건                       | FCP | LCP | 느린 무리로 갈린 회차 |
| -------------------------- | --- | --- | --------------------- |
| 워커 없음, 프리페치 그대로 | 96  | 532 | 8 / 12                |
| 워커, 프리페치 그대로      | 136 | 216 | 5 / 12                |
| 워커 없음, 프리페치 차단   | 172 | 224 | 1 / 12                |

마지막 열은 LCP 분포가 세 조건 모두 168~256ms의 빠른 무리와 360ms 위의 느린 무리로 이봉이라서 넣었다. 두 무리 사이에는 아무 회차도 없으므로 300ms를 경계로 세었고, 경계를 256과 360 사이 어디에 두어도 같은 수가 나온다.

프리페치를 막으면 워커 없이도 LCP가 224ms로 내려온다. 워커 쪽 216ms와 사실상 같다. 프리페치 트래픽이 LCP를 밀어낸다는 것까지는 이 표가 가른다. 차단 조건은 프리페치만 빼고 나머지가 워커 없는 조건과 같으니, 532에서 224로 내려온 몫은 프리페치의 것이다.

같은 표에서 워커 쪽 216ms를 읽을 때는 조심해야 한다. 두 조건이 같은 자리에 도착했다고 해서 같은 길로 간 것은 아니다. 방금 본 대로 워커는 프리페치 31건을 그대로 네트워크로 보낸다. 걷어 낸 것이 없는데 차단 조건과 같은 LCP가 나온 것이다.

정직하게 남길 것이 셋 있다. 하나는 워커가 무엇을 해서 LCP를 앞당겼는지를 끝내 못 짚었다는 것이다. 바이트가 아닌 것은 확인됐고, 남는 추정은 요청들이 워커를 경유하면서 우선순위나 렌더와의 경합 양상이 달라진다는 쪽인데 리소스 타이밍으로는 가르지 못했다. 같은 표의 FCP 열이 그 추정 쪽 방증으로 읽히기는 한다. LCP가 가장 나쁜 조건(워커 없음, 프리페치 그대로)의 FCP가 96ms로 셋 중 가장 좋고, LCP가 좋은 두 조건은 136ms와 172ms로 오히려 나쁘다. 순수한 바이트 경합이라면 나오기 어려운 패턴이라 렌더 순서 쪽 이야기로 보이지만, 이것도 추정이다. 둘은 `h1`이 왜 늦게 다시 그려지는지도 못 짚었다는 것이다. 느린 회차에는 첫 후보와 `h1` 사이에 중간 후보가 하나 더 끼고 `h1`이 뒤로 밀리는데, 그 재페인트를 무엇이 붙잡는지는 보이지 않았다. 프리페치를 걷어 내면 그 패턴이 12회 중 1회로 줄어든다는 것까지가 확인된 것이다. 셋은 워커가 완전 차단만큼 깨끗하지는 않다는 것이다. 워커 조건에서도 12회 중 5회는 밀렸다. 요청을 그대로 내보내면서 앞당기는 것과 아예 요청하지 않는 것은 같지 않다.
...
앞에서 정산한 바이트 비용과 나란히 놓으면 부호가 한 방향이라는 것도 분명해진다. 랩에서 잰 것은 빈 프로필로 소프트 내비게이션을 한 번 하는 상황이었고, 거기서는 워커가 HTML 한 벌과 썸네일 넉 장을 배경에서 더 받아 트래픽을 265KB 늘렸다. 방금 잰 것은 홈을 한 번 거친 뒤 하드 내비게이션을 하는 상황인데, 여기서는 워커가 프리페치 345KB를 걷어 내지도 늘리지도 않는다. 그대로 통과시킨다. 처음에는 이 두 장면을 부호가 반대인 수지로 정산하려고 했지만, 걷어 내는 쪽이 사라졌으니 정산할 것도 없다. 이 워커가 바이트에 하는 일은 늘리거나 같게 두거나 둘 중 하나다.

그러면 성능에는 도움이 되는가. 이 블로그에서는 된다. 웜 LCP 460ms 대 200ms는 짝비교 25쌍에서 나왔고, 체류 시간을 맞춘 스무 쌍과 순서를 뒤집은 열두 쌍에서도 방향이 그대로였다. 관측으로는 이만큼 단단하다.

왜 되는지는 모른다. 2편이 짚은 설명, 그러니까 정적 자산을 Cache Storage에 넣어 재방문 FCP를 줄인다는 쪽은 랩과 프로덕션 양쪽에서 접었다. 그다음으로 붙여 본 설명, 그러니까 HTTP 캐시가 재검증 없이는 내주지 못하는 RSC 프리페치를 워커가 대신 내준다는 쪽은 방금 코드와 데이터로 접었다. 이 워커는 프리페치를 대신 내주지 않는다. 남은 것은 추정뿐이라 이 자리는 비워 두는 것이 맞다.

그래서 1편이 서비스 워커를 쓸 이유로 꼽은 셋 가운데 세 번째, "HTTP 캐시로는 표현할 수 없는 전략이 필요할 때"에 이 블로그가 해당하는지도 아직 모른다. 그 조항에 해당한다고 말하려면 HTTP 캐시가 못 하는 일을 워커가 실제로 해내야 하는데, 이 워커는 그 자리에서 아무것도 하지 않고 요청을 흘려보낸다. RSC 프리페치가 HTTP 캐시로 잘 재사용되지 않는다는 사실은 남지만, 그 틈을 서비스 워커로 메울 수 있는지는 이 시리즈가 재지 않았다. 재려면 `handleRSC`에서 프리페치를 저장 대상에 넣고 캐시를 먼저 보게 고쳐야 하는데, 그때 `vary`가 갈리는 문제를 어떻게 다룰지가 다시 문제가 된다.

이어서 1편의 다른 문장 하나도 좁혀야 한다. "목표가 재방문 성능 하나라면 이 레이어는 답이 아닐 가능성이 높다"고 쓰면서 근거로 "HTTP 캐시가 이미 해주는 일을 코드로 다시 만드는 셈"이라고 했다. 앞부분은 이 블로그에서 확인됐다. 정적 자산에 관한 한 Cache Storage는 HTTP 캐시보다 빠르지 않았다. 흔들리는 것은 "이미 해주는 일"이라는 전제 쪽인데, 흔들린다는 것까지만 말할 수 있다. App Router의 RSC 프리페치처럼 HTTP 캐시가 원래 잘 못 해주는 일이 있다는 것은 헤더로 확인했고, 그것이 워커로 메워지는지는 확인하지 못했다.
...
| 브라우저 캐시가 재사용하지 못하는 요청이 페이지마다 수십 건씩 나간다 | 재 보고 정한다. 이 블로그는 웜 LCP가 460ms에서 200ms로 줄었지만, 워커가 그 요청을 대신 내준 결과는 아니었다  |
...
그래서 이 시리즈가 최종적으로 말할 수 있는 것은 이 정도다. 지연으로 치면 워커 경유 비용은 이 블로그의 중앙값에서 잡히지 않고, 꼬리에서는 여전히 크게 벌어지지만 그 꼬리가 워커 몫인지는 가르지 못했다. 바이트는 방문자가 무엇을 하느냐에 따라 늘거나 그대로다. 줄지는 않는다. 그런데도 프로덕션 웜 LCP는 절반 아래로 내려갔고, 그 이득이 어디서 오는지는 세 편을 다 쓰고도 답하지 못했다. navigation preload는 켜 두는 것이 맞고, 그것으로 회복되는 몫은 콜드 기동 한정이다. 그리고 이 숫자들을 얻는 데 든 노력의 대부분은 워커가 아니라 측정을 믿을 수 있게 만드는 데 들어갔다. 대조군이 자연히 생기지 않는다는 것을 확인하는 데 한 번, 프록시의 헤더와 인터셉션의 캐시와 콜드 기동의 정의에서 세 번 넘어진 뒤에야 첫 표가 나왔고, 그 표를 다 그리고 결론까지 쓴 뒤에 두 조건이 애초에 다른 순간을 재고 있었다는 것을 알았으며, 마지막에는 이득 쪽 표에서 `transferSize` 0을 캐시 적중으로 읽는 함정을 세 번째로 밟았다. 2편 끝에 "전후를 비교할 실사용자 지표 수집부터 갖추는 것이 순서"라고 적었는데, 이제는 두 문장을 덧붙이고 싶다. 실사용자 지표가 대조군을 주지 않을 때는 대조군을 만드는 비용까지가 그 기능의 비용이다. 그리고 대조군을 만들었다고 끝이 아니라, 양쪽 팔이 같은 것을 재고 있는지는 그다음 문제다.
