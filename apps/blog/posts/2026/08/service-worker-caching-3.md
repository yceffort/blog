...
세는 곳을 옮기면 직접 확인된다. 셰이핑 프록시에 붙인 카운터로 웜과 콜드 내비게이션에서 `/_next/static/` 요청이 몇 개나 회선을 지나는지 세 보니 두 조건이 정확히 같았다. 웜은 양쪽 다 15개 206,490바이트, 콜드는 양쪽 다 4개 150,609바이트였고 URL 집합까지 같았다. 워커 조건의 웜 내비게이션은 그 글에서 처음 쓰는 청크를 cacheFirst 미스로 흘려보냈고, 미스가 부른 `fetch()`는 그 아래의 HTTP 캐시를 그대로 썼다. Cache Storage가 실제로 답한 것은 콜드 쪽이다. 웜에서 저장해 둔 자산들이 브라우저 재기동을 넘어 쓰였다.
...
앞에서 잰 것은 전부 비용이다. 그런데 이 시리즈는 이득 쪽 숫자도 여러 번 인용해 놓고 그것을 따져본 적은 없다. 제목을 "비용 실측"으로 붙인 탓인데, 이대로 닫으면 절반만 보여주는 글이 된다.
...
TTFB 두 줄은 앞에서 본 Early Hints 아티팩트이니 지연으로 읽으면 안 된다. 읽을 것은 나머지 네 줄이고, 그중 웜 LCP가 예상 밖이었다. 랩에서는 워커를 켜고 끄는 것으로 LCP가 5ms도 움직이지 않았는데 프로덕션에서는 절반 아래로 내려간 것으로 보였다. 이 -260ms를 나는 뒤에 철회하게 된다. 값 자체는 25쌍에서 나온 진짜 p50이지만, p50으로 말해도 되는 분포가 아니었다.
...
두 조건 다 약 160에서 240 사이의 낮은 무리와 약 450에서 550 사이의 높은 무리로 갈리고, 그 사이에 있는 것은 356과 348 두 회차뿐이다. 조건이 바꾸는 것은 LCP 값이 아니라 높은 봉에 떨어질 확률이고, 그렇다면 중앙값은 그 확률이 절반을 넘느냐 아니냐에 따라 봉 사이를 건너뛴다. 실제로 워커 없음 25회의 p50 460ms를 같은 25개에서 부트스트랩으로 다시 뽑으면 95% 구간이 240에서 520이다. 한 데이터가 낮은 봉과 높은 봉 양쪽을 다 답으로 내놓는다. 그래서 웜 LCP는 p50이 아니라 봉 비율로 읽어야 한다. 350ms를 경계로 세면 웜은 워커 없음 17/25에 워커 1/25이고, 콜드는 24/25와 25/25다. 콜드는 양쪽 다 높은 봉에 붙어 있으니 596 대 528은 같은 봉 안의 차이이고, 조건이 가르는 것은 웜뿐이다.

먼저 내 측정 설계를 의심했다. 이 설계는 워커 조건만 홈에서 `controllerchange`를 기다리느라 3.5초를 더 머문다. 두 조건의 체류 시간을 맞추고 스무 쌍을 다시 돌려도, 조건 실행 순서를 뒤집어 열두 쌍을 더 돌려도 방향이 그대로였다(p50으로는 460 대 200과 524 대 200인데, 방금 본 이유로 이 값들도 봉 비율의 그림자로만 읽어야 한다). 설계 탓은 아니었다.
...
여기서 나는 이 글에서만 세 번째로 같은 함정을 밟았다. 0바이트를 캐시 적중으로 읽고 "워커가 이 31건을 Cache Storage에서 내준다"고 적었는데, `sw.js`를 열어 보면 그럴 수가 없다.
...
프리페치는 `isVisit`이 거짓이라 저장 대상에서 아예 빠지고, `cache.match()`는 `fetch()`가 던졌을 때만 들어가는 `catch` 안에 있다. 이 워커는 프리페치를 Cache Storage에서 내준 적이 없고, 켜져 있어도 31건을 그대로 네트워크로 보낸다. 0바이트는 함정 절에서 한 번, 정적 자산 절에서 또 한 번 경고해 놓은 그것, 워커를 지난 응답의 `transferSize`가 0으로 찍히는 계상 아티팩트다. 두 번을 적어 놓고 세 번째에 또 밟았다.

데이터도 같은 말을 한다. 세 조건의 원본을 다시 집계하면 이렇다.
...
워커 조건도 12회 전부 31건이 나갔고, 마지막 응답이 끝난 시각이 1,142ms 대 1,115ms로 사실상 같다. 캐시에서 꺼냈다면 나올 수 없는 값이다. 차단 조건의 0바이트는 또 다른 이유로 0인데, 막힌 요청도 `transferSize`가 0으로 찍히기 때문이다. 앞 절에서 프리페치 325건이 워커를 지나는 데 1ms밖에 안 들고 duration이 미경유와 같았던 것도 이제 다르게 읽힌다. 빨라서가 아니라 그냥 지나간 것이다.
...
`no-store`는 없으니 브라우저 캐시는 이 응답을 저장은 한다. 다만 `max-age=0, must-revalidate`라 쓸 때마다 서버에 재검증 왕복을 한 번 해야 하고, 그 왕복이 남는 이상 프리페치의 값어치는 대부분 사라진다. 게다가 `vary`에 걸린 넷 가운데 `next-router-state-tree`는 출발한 페이지마다 값이 달라진다. 같은 대상 경로라도 어디서 이동해 왔느냐에 따라 캐시 항목이 갈리니, 저장은 되는데 다시 맞을 일이 드물다. 이 응답을 재사용하는 캐시가 아주 없는 것은 아니다. `x-nextjs-stale-time: 300`은 App Router가 자체 라우터 캐시에 5분 동안 들고 있겠다는 뜻인데, 그 캐시는 페이지를 새로 열면 사라지는 메모리 캐시다. 디스크에 남는 것은 Cache Storage뿐인데, Cache Storage의 저장과 조회 알고리즘에는 `cache-control`도 `x-nextjs-stale-time`도 나오지 않는다. 조회에서 보는 것은 메서드와 URL, 그리고 `ignoreVary`를 켜지 않았다면 `vary`다. 그러니까 Cache Storage에 넣어 두더라도 `vary`가 갈리는 문제는 그대로 따라온다.

여기까지는 상관이다. 이 시리즈는 상관을 인과로 읽었다가 세 번 되돌렸으니 한 번 더 갈라야 했다. 앞의 세 조건에 하나를 더해 넷으로 만들고 조건당 30회로 다시 돌렸다. 워커 없이 프리페치 그대로, 배포된 워커 그대로, 배포된 워커에서 `handleRSC`만 프리페치도 저장하고 조회하는 변형으로 갈아끼운 것, 그리고 워커 없이 프리페치만 막은 것이다. 차단은 CDP의 `Network.setBlockedURLs`로 했다. Playwright의 `route()`는 앞의 함정에서 본 대로 HTTP 캐시를 꺼서 워커 없는 조건만 불리해지니 쓸 수 없었고, 대신 Network 도메인 자체는 네 조건 모두에서 켜 두어 도메인 활성화를 상수로 만들었다. 회차마다 조건 실행 순서를 돌렸다.

변형을 심는 데도 한 번 돌아갔다. `route()`로 `/sw.js` 응답을 바꿔치기하는 방법은 실측으로 폐기했다. Chrome은 내비게이션마다 워커 스크립트를 다시 받아 비교하는 soft update를 도는데, 그 요청이 Playwright `route()`로도 페이지 타깃 CDP `Fetch`로도 브라우저 타깃 CDP `Fetch`로도 가로채지지 않아서 변형이 한두 번 만에 배포본으로 되돌아갔다. 대신 `sw.js`는 classic worker라 최상위 함수 선언이 워커 전역 객체의 프로퍼티가 된다. `worker.evaluate()`로 `self.handleRSC`만 갈아끼우면 두 워커 조건이 배포된 같은 파일을 그대로 돌리면서 그 함수 하나만 달라지고, `route()`를 아예 쓰지 않으니 캐시를 끄는 함정도 따라오지 않는다.

한 가지 더 맞춰 줘야 했다. `_rsc` URL은 출발 페이지에 따라 갈리므로, 변형이 캐시에서 내주려면 같은 출발 페이지에서 같은 글로 한 번 다녀온 적이 있어야 한다. 그래서 네 조건 모두 글을 한 번 방문해 두고, HTTP 캐시와 RSC 캐시를 뺀 나머지 워커 캐시를 지운 뒤 두 번째 방문을 쟀다. 변형에게 가장 유리한 상황이라는 뜻이다.

| 조건                     | FCP p50 | LCP p50 | 높은 봉    |
| ------------------------ | ------- | ------- | ---------- |
...
(단위 ms, 조건당 30회, 120회 전부 엣지 HIT. 봉의 경계는 앞과 같은 350ms이고, 324와 416 사이에는 아무 회차도 없다. LCP p50 열은 참고로만 두었다. 배포본의 320ms는 부트스트랩 95% 구간이 164에서 452라 같은 데이터가 두 봉 다 답으로 낸다.)

변형이 정말로 캐시에서 내줬는지는 `transferSize`로 확인하지 않았다. 이 글에서 세 번 밟은 함정이 그것이다. 워커 안에 카운터를 두고 프리페치의 캐시 적중과 네트워크 fetch를 직접 셌다. 배포본은 적중 0에 미스 31에 네트워크 31이었고 변형은 적중 31에 미스 0에 네트워크 0이었으며, 30회 전부 그랬다. 리소스 타이밍도 같은 말을 한다. 프리페치 31건의 duration 합이 배포본 2,104ms 대 변형 30ms이고, 마지막 프리페치 응답이 끝난 시각이 1,224ms 대 863ms다.

읽을 것은 둘이다. 하나는 프리페치를 캐시에서 내주면 아예 막은 것과 같은 자리에 도착한다는 것이다. 9/30과 7/30은 카이제곱으로 p = 0.77이라 구별되지 않는다. 프리페치 트래픽이 높은 봉을 만든다는 것은 워커 없음 20/30 대 차단 7/30에서 p = 0.002로 이미 갈렸고(차단 조건은 프리페치만 빼고 나머지가 워커 없는 조건과 같다), 변형은 그 트래픽을 회선에서 걷어 내는 다른 방법이 같은 곳에 도착한다는 것을 보여준다.
...
정직하게 남길 것이 셋 있다. 하나는 배포본이 프리페치를 한 건도 걷어 내지 않으면서 무엇을 해서 20/30을 14/30으로 만들었는지를 끝내 못 짚었다는 것이다. 바이트가 아닌 것은 확인됐고, 남는 추정은 요청들이 워커를 경유하면서 우선순위나 렌더와의 경합 양상이 달라진다는 쪽인데 리소스 타이밍으로는 가르지 못했다. 같은 표의 FCP 열이 그 추정 쪽 방증으로 읽히기는 한다. 높은 봉이 가장 잦은 조건의 FCP가 96ms로 넷 중 가장 좋고, 봉이 가장 드문 두 조건은 104ms와 128ms로 오히려 나쁘다. 순수한 바이트 경합이라면 나오기 어려운 패턴이라 렌더 순서 쪽 이야기로 보이지만, 이것도 추정이다.

둘은 높은 봉의 정체를 못 짚었다는 것이다. 의심할 만한 후보는 있었다. `tailwind.css`의 `::view-transition-old(root)`와 `::view-transition-new(root)`가 `animation-duration: 0.3s`이고 포스트 `h1`이 `<ViewTransition>`으로 감싸져 있으며, 관측된 계단이 약 310ms였다. 그래서 워커 유무와 뷰 트랜지션 유무를 2×2로 놓고 칸당 22회를 쟀다. `startViewTransition`을 스텁으로 갈아끼우면 스냅샷과 커밋 타이밍까지 함께 바뀌므로, 메커니즘은 그대로 두고 애니메이션 길이만 1ms로 덮었다. 조작은 걸렸다. 트랜지션은 네 칸 모두 세 번씩 그대로 일어났고 호출 간격 p50만 373ms에서 92ms로 줄었다. 그런데 높은 봉은 줄기는커녕 합산 34%에서 48%로 늘었다(p = 0.28이라 이쪽도 유의하지는 않다). 결정적인 것은 봉의 위치다. 애니메이션을 끄면 세 번의 트랜지션이 300\~\~400ms에 다 끝나는데 높은 봉 LCP는 508\~\~584ms이고, 켜면 마지막 호출이 830ms인데 높은 봉은 448\~\~548ms다. 봉은 트랜지션 일정과 무관하게 절대 시각 450\~\~580ms에 고정돼 있었다. 기각이다.

그 2×2가 남긴 단서가 하나 있다. 네 칸 전부에서 높은 봉 회차의 FCP가 오히려 빠르다(88\~\~92ms 대 104\~\~120ms). 봉을 가르는 것이 "느려서 늦다"가 아니라 첫 페인트에 `h1`이 들어갔느냐로 보인다는 뜻이다. 낮은 봉은 첫 페인트가 조금 늦되 `h1`을 포함해 FCP와 LCP가 거의 붙고, 높은 봉은 `h1` 앞에 다른 것이 먼저 그려져 FCP가 일찍 찍히고 `h1`은 두 번째 페인트로 밀린다. 그렇다면 남은 후보는 스트리밍과 하이드레이션 순서다. `next.config.ts`에 `cacheComponents: true`가 켜져 있고 포스트 라우트에 `loading.tsx`가 있으니 셸이 먼저 오고 본문이 나중에 붙는 구조이기는 하다. 다만 이쪽은 재 보지 않았다. 추정이다.
...
앞에서 정산한 바이트 비용과 나란히 놓으면 부호가 한 방향이라는 것도 분명해진다. 랩에서 잰 것은 빈 프로필로 소프트 내비게이션을 한 번 하는 상황이었고, 거기서는 워커가 HTML 한 벌과 썸네일 넉 장을 배경에서 더 받아 트래픽을 265KB 늘렸다. 방금 잰 것은 홈을 한 번 거친 뒤 하드 내비게이션을 하는 상황인데, 여기서는 워커가 프리페치 345KB를 걷어 내지도 늘리지도 않는다. 그대로 통과시킨다. 처음에는 이 두 장면을 부호가 반대인 수지로 정산하려고 했지만, 걷어 내는 쪽이 사라졌으니 정산할 것도 없다. 지금 짜인 대로라면 이 워커가 바이트에 하는 일은 늘리거나 같게 두거나 둘 중 하나다. 걷어 내는 쪽은 짜기 나름이라는 것이 변형 조건이 보여준 것이다.

그러면 성능에는 도움이 되는가. 이 블로그에서는 된다. 웜 LCP가 높은 봉에 떨어지는 비율이 워커 없음 17/25에서 워커 1/25로 내려갔고, 체류 시간을 맞춘 스무 쌍과 순서를 뒤집은 열두 쌍, 그리고 다른 날 다시 돌린 30회짜리 네 조건(20/30 대 14/30)에서도 부호가 그대로였다. 크기는 실행마다 흔들리니 말할 수 없다. 관측으로 단단한 것은 방향까지다.

왜 되는지는 모른다. 2편이 짚은 설명, 그러니까 정적 자산을 Cache Storage에 넣어 재방문 FCP를 줄인다는 쪽은 랩과 프로덕션 양쪽에서 접었다. 그다음으로 붙여 본 설명, 그러니까 HTTP 캐시가 재검증 없이는 내주지 못하는 RSC 프리페치를 워커가 대신 내준다는 쪽은 코드와 카운터로 접었다. 이 워커는 프리페치를 대신 내주지 않는다. 대신 내주는 변형이 더 내려간다는 것은 확인했지만, 배포본이 아무것도 걷어 내지 않고도 내려가 있는 몫은 여전히 설명이 없다. 그 자리는 비워 두는 것이 맞다.

그래서 1편이 서비스 워커를 쓸 이유로 꼽은 셋 가운데 세 번째, "HTTP 캐시로는 표현할 수 없는 전략이 필요할 때"에 이 블로그가 해당하는지는 이제 반쯤 답이 있다. `handleRSC`를 프리페치도 저장하고 조회하도록 고친 변형은 프리페치 31건을 전부 캐시에서 내줬고, 높은 봉 비율이 프리페치를 아예 차단한 조건과 구별되지 않는 곳까지 내려갔다. HTTP 캐시가 재검증 없이는 못 하는 일을 워커가 해낸 것이다. 다만 이 측정은 변형에게 가장 유리하게 짜여 있었다. 같은 출발 페이지에서 같은 글로 두 번째 들어가는 상황이라 `_rsc` URL이 첫 방문과 같았기 때문이다. 실제 독자는 매번 다른 곳에서 들어오고 그때마다 `next-router-state-tree`가 달라지니, 적중률이 이만큼 나올지는 재지 않았다.

이어서 1편의 다른 문장 하나도 좁혀야 한다. "목표가 재방문 성능 하나라면 이 레이어는 답이 아닐 가능성이 높다"고 쓰면서 근거로 "HTTP 캐시가 이미 해주는 일을 코드로 다시 만드는 셈"이라고 했다. 앞부분은 이 블로그에서 확인됐다. 정적 자산에 관한 한 Cache Storage는 HTTP 캐시보다 빠르지 않았다. 흔들리는 것은 "이미 해주는 일"이라는 전제 쪽이다. App Router의 RSC 프리페치처럼 HTTP 캐시가 원래 잘 못 해주는 일이 있다는 것은 헤더로 확인했고, 그 자리를 워커가 메울 수 있다는 것도 한 번은 확인했다. 확인하지 못한 것은 그 이득이 실제 이동 패턴에서도 남느냐다.

이 결론이 어디까지 서는지도 밝혀 둬야 한다. 이 블로그는 CDN이 이미 빠르고(엣지 HIT에서 최종 헤더까지 46ms다), 정적 자산에 `immutable` 캐시 헤더가 제대로 박혀 있고, LCP 요소가 텍스트다. 세 조건이 다르면 답도 다르다. 특히 원본 서버가 느리거나 CDN이 없는 사이트라면 Cache Storage가 지우는 왕복의 크기 자체가 달라져서 정적 자산 쪽에서도 이득이 날 수 있는데, 이 시리즈는 그런 환경을 한 번도 재지 않았다. 여기서 얻은 것은 "서비스 워커는 성능에 쓸모없다"가 아니라 "이 조건에서는 이득이 났고, 그 이득이 어디서 오는지는 아직 못 짚었다"이다.
...
| 브라우저 캐시가 재사용하지 못하는 요청이 페이지마다 수십 건씩 나간다 | 재 보고 정한다. 이 블로그는 웜 LCP가 느린 봉에 떨어지는 비율이 내려갔지만, 배포본이 그 요청을 대신 내준 결과는 아니었다 |
...
그래서 이 시리즈가 최종적으로 말할 수 있는 것은 이 정도다. 지연으로 치면 워커 경유 비용은 이 블로그의 중앙값에서 잡히지 않고, 꼬리에서는 여전히 크게 벌어지지만 그 꼬리가 워커 몫인지는 가르지 못했다. 바이트는 방문자가 무엇을 하느냐에 따라 늘거나 그대로다. 줄지는 않는다. 그런데도 프로덕션 웜 LCP가 느린 봉에 떨어지는 비율은 워커를 켰을 때 분명히 내려갔고, 그 이득이 어디서 오는지는 세 편을 다 쓰고도 답하지 못했다. 프리페치까지 캐시에서 내주도록 고치면 더 내려간다는 것이 이번에 얻은 유일한 처방인데, 그 자리를 막고 있던 것이 2편에서 스스로 넣은 저장 가드였다. navigation preload는 켜 두는 것이 맞고, 그것으로 회복되는 몫은 콜드 기동 한정이다. 그리고 이 숫자들을 얻는 데 든 노력의 대부분은 워커가 아니라 측정을 믿을 수 있게 만드는 데 들어갔다. 대조군이 자연히 생기지 않는다는 것을 확인하는 데 한 번, 프록시의 헤더와 인터셉션의 캐시와 콜드 기동의 정의에서 세 번 넘어진 뒤에야 첫 표가 나왔고, 그 표를 다 그리고 결론까지 쓴 뒤에 두 조건이 애초에 다른 순간을 재고 있었다는 것을 알았으며, 이득 쪽 표에서는 `transferSize` 0을 캐시 적중으로 읽는 함정을 세 번째로 밟았다. 마지막 하나는 이 편이 스스로 판 것이다. 2편의 평균이 41.7초짜리 극단값에 끌리는 것을 보고 백분위수로 갈아탄 것이 이 편의 첫 번째 시도였는데, 그 백분위수가 이봉분포에서 봉 사이를 건너뛰었다. 평균보다 나은 통계량을 골랐다는 것이 분포를 안 봐도 된다는 뜻은 아니었다. 2편 끝에 "전후를 비교할 실사용자 지표 수집부터 갖추는 것이 순서"라고 적었는데, 이제는 두 문장을 덧붙이고 싶다. 실사용자 지표가 대조군을 주지 않을 때는 대조군을 만드는 비용까지가 그 기능의 비용이다. 그리고 대조군을 만들었다고 끝이 아니라, 양쪽 팔이 같은 것을 재고 있는지는 그다음 문제다.
