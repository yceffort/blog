...
그리고 이 조사에는 사실 지름길이 있었다. 문제가 나타나기 직전에 바뀐 것은 Next.js 버전업뿐이었다. "설마 프레임워크 버전업 때문이겠어"라고 넘긴 그 판단 탓에 조사가 어떻게 하루짜리가 됐는지도 뒤에서 같이 정리한다.
...
검증하려고 배포된 청크를 긁었는데 여기서 한 번 넘어졌다. 553개 청크 대부분이 403으로 막혀 있었고, 실제로 검사된 것은 극소수였다. 그런데 이걸 "전수 스캔 결과 socket은 1개"로 읽어버렸다. 도구가 실패한 것을 사실이 부재하는 것으로 취급한 오판이고, 이 오판 때문에 정답(인스턴스가 두 벌이라는 가설)을 한 번 버렸다가 되찾았다.

돌파구는 배포본을 긁는 대신 로컬에서 같은 조건으로 빌드하는 것이었다. 문제가 재현되던 패키지 버전으로 고정하고 같은 환경 변수의 빌드 스크립트를 turbopack으로 돌리면, 문제의 socket이 담긴 청크를 포함해 **파일명이 배포본과 일치하는 산출물**이 나왔다. 403도 없고 전수 검사가 자유롭다. 산출물의 청크 277개를 전부 grep했다. (배포 스캔의 553개와 로컬 산출물 277개가 왜 다른지는 끝까지 규명하지 않았다. 다만 반증에 필요한 것은 배포본과의 개수 맞추기가 아니라 문제 빌드 산출물의 전수 검사이고, 로컬 빌드가 그것을 준다.)
...
가장 큰 것은 이미 쓴 대로 용의자 선정의 실패다. 가장 최근에 바뀐 것(프레임워크 버전)을 검증 없이 용의선상에서 내렸다. 경험칙이 대개 옳다는 사실이 검증을 생략할 이유는 되지 않는다는 것, 특히 실험을 용의 목록에 올려보는 비용이 0일 때는 더욱 그렇다는 것을 배웠다.
...
잘못된 반증도 두 번 했다. "스택트레이스 오프셋이 같으니 모듈은 한 벌"(같은 코드를 공유하면서 모듈 레코드만 별개일 수 있으므로 틀렸다), "청크 스캔 결과 socket은 1개"(대부분 403이라 실제로는 스캔이 안 됐다). 둘 다 도구와 방법의 한계를 사실로 취급한 것이고, 이 두 번의 오판으로 정답을 두 번 버렸다가 되찾았다. 없는 파일에서 못 찾은 것과 있는 파일에서 못 찾은 것은 다르다.
