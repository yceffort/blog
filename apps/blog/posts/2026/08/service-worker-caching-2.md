...
TTFB 악화(재방문자 평균 +525ms)는 1편에서 다룬 워커 기동·fetch 경유 오버헤드가 주범으로 보이지만, 위에서 유보한 대로 전액 귀속은 `sw_controlled` 계측으로 확인할 일이 남았다. 워커 기동을 기다리지 않고 내비게이션 요청을 먼저 출발시키는 navigation preload가 이 숫자를 회복할 다음 과제로 남아 있다.
