...
The two TTFB rows are the Early Hints artifact from earlier and must not be read as latency. The other four are what to read, and the warm LCP was not what I expected. In the lab, toggling the worker moved LCP by less than 5 ms; in production it appeared to drop to less than half. That -260 ms is one I go on to withdraw. The value is a real p50 over 25 pairs, but this is not a distribution a p50 is allowed to describe.
...
Both conditions split into a low cluster around 160 to 240 and a high cluster around 450 to 550, with only two runs in between, at 356 and 348. What the condition changes is not the LCP value but the probability of landing on the high peak, and a median under those circumstances hops between peaks depending on whether that probability crosses one half. Bootstrap the no-worker p50 of 460 ms out of those same 25 values and the 95% interval runs from 240 to 520. One dataset gives both peaks as its answer. So warm LCP has to be read as a peak ratio, not a p50. Counting at a 350 ms boundary, warm is 17/25 for no worker against 1/25 for the worker, and cold is 24/25 against 25/25. Cold sits on the high peak in both conditions, so 596 against 528 is a difference within one peak, and warm is the only phase the condition separates.

My first suspect was my own apparatus. Only the worker condition dwells an extra 3.5 seconds on the home page waiting for `controllerchange`. Matching the dwell across conditions and running twenty more pairs, and reversing the order in which the conditions run for another twelve pairs, both kept the direction (as p50s, 460 against 200 and 524 against 200, though for the reason just given those too are only shadows of the peak ratio). Not the apparatus.
...
| Condition                     | FCP p50 | LCP p50 | High peak  |
| ----------------------------- | ------- | ------- | ---------- |
...
Three things stay honest. First, I never pinned down what the deployed worker does to turn 20/30 into 14/30 while taking not one prefetch off the wire. That it is not bytes is confirmed; what remains is a guess, that routing these requests through the worker changes their priority or how they contend with rendering, and resource timings cannot separate that. The FCP column in the same table reads as circumstantial support for the guess. The condition where the high peak is most frequent has the best FCP of the four at 96 ms, while the two where it is rarest are worse at 104 ms and 128 ms. That pattern is hard to get from pure byte contention, which points at render ordering, but that too is a guess.

Second, I never pinned down what the high peak actually is. There was a candidate worth suspecting. `tailwind.css` gives `::view-transition-old(root)` and `::view-transition-new(root)` an `animation-duration: 0.3s`, the article `h1` is wrapped in `<ViewTransition>`, and the step I was seeing was about 310 ms. So I put worker on/off against view transitions on/off in a 2x2 and ran 22 per cell. Swapping `startViewTransition` for a stub would change snapshot and commit timing along with it, so I left the mechanism alone and overrode only the animation duration to 1 ms. The manipulation took: transitions still fired three times in every cell, and only the interval between calls dropped from a p50 of 373 ms to 92 ms. Yet the high peak did not shrink; it grew, from 34% to 48% in aggregate (p = 0.28, so not significant either). The decisive part is where the peak sits. With the animation off, all three transitions finish between 300 and 400 ms while high-peak LCP is 508 to 584 ms; with it on, the last call is at 830 ms while the high peak is 448 to 548 ms. The peak is pinned to an absolute 450 to 580 ms regardless of the transition schedule. Rejected.

That 2x2 did leave one clue. In all four cells, the high-peak runs have the _faster_ FCP (88 to 92 ms against 104 to 120 ms). Which suggests the split is not "slow, therefore late" but whether the `h1` made it into the first paint. On the low peak the first paint comes a little later but includes the `h1`, so FCP and LCP nearly coincide; on the high peak something else is painted before the `h1`, so FCP lands early and the `h1` slips to a second paint. If so, what remains are streaming and hydration order. `next.config.ts` has `cacheComponents: true` and the article route has a `loading.tsx`, so the shell does arrive before the body. But I did not measure this side. It is a guess.
...
| Requests the browser cache cannot reuse go out by the dozen on every page | Measure, then decide. Warm LCP on this blog landed on the slow peak less often, but not because the deployed worker served those requests instead |
...
So this is about as much as the series can finally say. In latency, going through the worker does not show up at this blog's median, and the tail still spreads wide, but whether that tail belongs to the worker is something I could not separate out. In bytes it goes up or stays level depending on what the visitor does; it never goes down. And yet production warm LCP landed on the slow peak distinctly less often with the worker on, and after three posts I still cannot say where that came from. The one prescription this round produced is that fixing the worker to serve prefetches from cache lowers it further, and what was blocking that spot was the storage guard I put in myself in part 2. Navigation preload should stay on, and what it recovers is limited to cold starts. And most of the effort behind these numbers went not into the worker but into making the measurement trustworthy. It took one round to confirm that a control group would not form on its own, and three falls, over the proxy's headers, the interception's cache, and the definition of a cold start, before the first table appeared; it was only after every table was drawn and the conclusion written that I learned the two conditions had been timing different moments all along; and on the benefit table I walked into the trap of reading `transferSize` 0 as a cache hit for the third time. The last one this post dug for itself. Watching part 2's average get dragged by a single 41.7-second outlier is what made me switch to percentiles, which is where this post's first attempt begins, and that percentile then hopped between the peaks of a bimodal distribution. Picking a better statistic than the mean did not mean I was excused from looking at the distribution. Part 2 closed by saying that collecting real user metrics for a before-and-after comparison should come first; I would now add two sentences. When the real user metrics do not give you a control group, the cost of building one is part of the feature's cost. And building one is not the end of it: whether both arms are measuring the same thing is the next question.
