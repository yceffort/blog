...
So when should you reach for one? Part 1 named three conditions, and having measured everything in this series, two of them stand as written and the third can now carry a test. If offline is a requirement, performance does not enter into it. The same goes for users on unreliable networks, though as part 2 admitted, this implementation has no timeout fallback and is helpless under lie-fi, so that case calls for putting a timeout on network-first before anything else. The hard one was the third. Needing a strategy the HTTP cache cannot express is impossible to check against your own site while it stays a generality. Now I know where to look. Open DevTools, pick the requests that go out repeatedly on every page, and read their response headers. If `max-age=0` or `must-revalidate` is there and `vary` lists several headers, the browser cache cannot serve that request without revalidating, and if dozens of them go out per page you are in the same situation this blog was. App Router's RSC prefetches were exactly that. The more prefetches a framework fires on its own, the more likely this clause applies.

The other way round: if the goal is returning-visit performance on static assets alone, this series says do not. Hashed assets with `immutable` already settle it, and Cache Storage was no faster than the HTTP cache in the lab or in production.
