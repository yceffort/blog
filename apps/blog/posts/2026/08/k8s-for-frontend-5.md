...
사슬을 풀면 이렇다. 과부하로 응답 큐가 깊어지면, readiness probe의 요청도 같은 이벤트 루프에 줄을 선다. probe의 기본 타임아웃은 1초라, 앱이 멀쩡히 일하고 있어도 검사는 3연속 실패하고 파드는 NotReady가 된다([3편](/2026/08/k8s-for-frontend-3)의 탈락 타임라인 그대로다). 그런데 HPA의 사용률 계산은 **Ready가 아닌 파드를 표본에서 뺀다.** 파드 3개가 전부 NotReady면 표본이 0개가 되고, HPA는 "invalid metrics"를 기록하며 판단을 보류한다. 확장의 근거 데이터가, 확장이 필요한 바로 그 상황 때문에 사라지는 구조다. 관찰 내내 probe는 성공과 실패를 오갔고(파드 Ready 조건이 여러 번 반전된 것이 conditions에 남아 있다), 어쩌다 메트릭이 잡힌 순간에야 desired가 움직였다.

[4편](/2026/08/k8s-for-frontend-4)에서 liveness에 의존성을 걸면 장애가 재시작 폭풍이 된다고 했는데, 이것은 그 readiness 판이다. probe가 서빙과 같은 큐를 쓰는 한, 깊은 과부하는 트래픽 이탈과 오토스케일러 실명을 동시에 부른다. liveness였다면 여기에 재시작까지 겹쳤을 것이다.

그럼 실명은 어떤 조건에서 오는가. 유입 상한을 둔 개루프 부하로 강도를 바꿔 가며 경계를 찾아봤다.

| 개루프 스파이크 (probe timeout 기본 1초) | 143rps (한계의 약 1.2배) | 222rps (한계의 약 1.9배)   |
| ---------------------------------------- | ------------------------ | -------------------------- |
| desired 첫 변화                          | +32.2초 (정상 감지)      | **동결** (부하 210초 내내) |
| p95 최악 구간                            | 1.6초                    | 4.5초                      |
| 실패율                                   | **0%** (0/30,064)        | **27.0%** (7,017/25,972)   |

143rps에서는 실명이 없었다. 큐가 1.6초까지 밀렸지만 probe가 다 죽기 전에(NotReady 판정에는 5초 간격 3연속 실패, 최소 십수 초가 필요하다) 감지 창이 먼저 끝나 확장이 이겼다. 반면 222rps에서는 같은 probe 1초가 부하 내내 desired를 동결시켰고, 실패가 27%까지 쌓였다. 즉 실명은 강도의 문제라기보다 **경주의 문제다.** 감지 창(15~30초)이 끝나기 전에 큐가 probe를 죽일 만큼 깊어지면 오토스케일러는 눈을 잃고, 그 반대면 산다.

경주에서 probe 쪽에 시간을 벌어 주는 처방이 timeoutSeconds 상향이다. readiness의 타임아웃을 1초에서 3초로 올리고 같은 222rps를 반복했다.

| 같은 222rps 스파이크 | probe timeout 1초          | probe timeout 3초        |
| -------------------- | -------------------------- | ------------------------ |
| desired 첫 변화      | **동결** (부하 210초 내내) | +197.0초 (부하 후반에야) |
| p95 최악 구간        | 4.5초                      | 2.3초                    |
| 실패율               | **27.0%** (7,017/25,972)   | 19.1% (7,342/38,399)     |

타임아웃 상향은 실패율을 27%에서 19%로 낮추고 부하 후반에는 확장을 되살렸지만, 실명 자체를 막지는 못했다. 물론 타임아웃 상향은 경주의 판돈을 키울 뿐 구조를 바꾸지는 못한다. 큐가 3초를 넘겨 밀리는 부하 앞에서는 같은 일이 반복된다. 더 근본적으로는 probe 응답이 서빙 큐를 우회하게 만들거나 유입 쪽의 백프레셔가 필요한데, 그 설계는 이 글의 범위를 넘는다. 여기서 가져갈 것은 진단 쪽이다. **스파이크 때 HPA가 안 움직였다면 desired가 아니라 `kubectl describe hpa`의 이벤트부터 볼 것.** "pods might be unready"가 찍혀 있다면 범인은 HPA 설정이 아니라 probe다.
...
| -------------------- | ----------------------- | ---------------------------------- |
| p95 최악 구간        | 18.7초 (실명 동반)      | **101ms** (내내 두 자릿수)         |
| 실패율               | 27%                     | **0%** (0/30,094)                  |
| 스케일 아웃 대기     | 146.7초                 | **0초 (이미 떠 있음)**             |

같은 강도의 부하가 도착했을 때 이미 9개가 떠 있으니, 감지도 기동도 없이 p95가 두 자릿수에 머물렀다. 감지 창과 실명 문제를 실측으로 본 뒤라면 이 대조가 말하는 바는 분명하다. **가장 빠른 스케일 아웃은 스파이크가 오기 전에 끝난 스케일 아웃이다.** 물론 시각을 모르는 스파이크에는 통하지 않는 방법이고, 그때 돌아갈 곳은 다시 감지 창을 버틸 여유, 즉 minReplicas와 requests다.
...
- **개입은 양끝에서만 통한다.** 미리 띄우기(minReplicas)는 한계 상황의 보험이고, 몰아서 띄우기(behavior)는 기동 경합으로 역효과가 났으며, 이미지 준비(크기·프리로드)는 콜드 노드의 7.7초(naive)와 0.23초(standalone)를 가른다.
- **과부하는 오토스케일러의 눈을 가린다.** probe가 서빙 큐에 밀려 전 파드 NotReady가 되면 HPA는 표본을 잃는다. 한계를 한참 넘긴 부하에서 146.7초 실명에 실패 27%를 관찰했고, 실명 여부는 감지 창과 probe 붕괴의 경주로 갈렸다. 같은 222rps에서 probe 타임아웃 상향(1→3초)은 실패를 27%에서 19%로 줄였을 뿐 실명을 막지 못했으니, 진짜 방어는 감지 창을 버틸 용량이다. 진단은 describe hpa의 "pods might be unready"부터.
- **스케일 다운은 304초의 계단이다.** 안정화 창 300초의 롤링 맥스가 12→8→3 계단을 만들었고, 내려가는 파드는 [4편](/2026/08/k8s-for-frontend-4)의 종료 시퀀스를 탄다.
- **메모리는 Node에서 스케일링 신호가 못 된다.** 부하 3분에 2→8로 늘어난 파드가, V8이 반납하지 않는 RSS와 새 파드의 평균 희석 때문에 부하가 빠진 뒤에도 8개로 고착됐다.
- **시각을 아는 스파이크는 감지를 건너뛴다.** KEDA cron 선제 확장은 같은 브루탈 스파이크를 에러 0, p95 최악 101ms로 받아냈다.
