...
To be precise, this blog never had the problem at that scale. The post links in the list, card, search, and archive components had carried `prefetch={false}` for other reasons since a month or so before the service worker was built. Counting the requests that go out while moving from the home page into a post, the only prefetches on post paths are the English toggle link and the next-post link in the series; every other prefetch goes to `/tags/*` and `/series/*`. Filtering was still not optional. An unread next post showing up as "saved for offline" is the same kind of problem as dozens of them piling up.
