...
일반 웹 글꼴을 제거하고 다시 빌드한 뒤, 수식 글의 첫 방문 LCP 중앙값은 5,650ms에서 2,352ms로 줄었다. FCP는 1,514ms에서 758ms가 됐다. 수식 전용 글꼴은 유지했고, 수정 후 네 번 모두 5초대 LCP가 나오지 않았다. 다만 홈 LCP는 측정 대상이 바뀌며 늦어졌으므로 모든 경로의 지표가 좋아졌다고 쓰지는 않는다.
...
xychart-beta horizontal
    ...
    x-axis ["홈 전", "홈 후", "코드 전", "코드 후", "수식 전", "수식 후"]
    y-axis "ms" 0 --> 2800
    bar [1556, 1568, 1898, 2014, 1658, 1450]
```
...
xychart-beta horizontal
    ...
    x-axis ["홈 전", "홈 후", "코드 전", "코드 후", "수식 전", "수식 후"]
    y-axis "ms" 0 --> 7500
    bar [1644, 1648, 3242, 3082, 2428, 5640]
```
...
---
xychart-beta horizontal
    title "첫 방문 본문 DOM 감지"
    x-axis ["코드 전", "코드 후", "수식 전", "수식 후"]
    y-axis "ms" 0 --> 2100
    bar [1487.8, 1472.8, 1277.3, 1099.4]
```
...
기존 네 번의 추적 기록에서는 비슷한 순서가 반복됐다. 약 1.10초에 본문 DOM이 들어오고, 1.22초에 Libertinus Math 요청이 시작됐다. 글꼴 로드는 약 5.57초에 끝났고, 약 70ms 뒤 본문 문단이 LCP로 기록됐다. 수식 글꼴 전송량도 마이그레이션 전 약 64KiB에서 334KiB로 늘었다. 다만 시간 순서만 보고 글꼴이 원인이라고 결론 내릴 수는 없었다.
...
flowchart LR
    ...
    source --> condition0["기존 설정<br/>5,636ms (문단)<br/>1,552ms (제목)<br/>5,636ms (문단)"]
    source --> condition1["수식 글꼴 즉시 전달<br/>2,804ms (배너)<br/>2,792ms (배너)<br/>1,524ms (제목)"]
    source --> condition2["수식 글꼴 요청 1.5초 지연<br/>1,528ms (제목)<br/>6,020ms (문단)<br/>6,028ms (문단)"]
...
수식 글꼴을 즉시 전달하면 세 번 모두 5초대의 늦은 문단 후보가 나오지 않았다. 요청을 늦춘 조건에서는 문단이 최종 후보인 두 번이 6,020ms와 6,028ms로 더 늦어졌다. 나머지 한 번은 제목이 최종 후보로 남아 1,528ms였다. 원래 설정에서도 한 번은 제목이 LCP로 남아 1,552ms에 끝났다. 화면 전환을 끈 세 번에서는 모두 5초대의 늦은 문단 후보가 다시 나왔으므로, 화면 전환이 지연을 일으킨다는 가설은 버렸다. 글꼴 전송 시점은 늦은 LCP 후보에 영향을 줬지만, 최종 값은 어떤 요소가 가장 큰 후보로 남는지에도 달려 있었다.
...
이번 수정으로 일반 글꼴 약 168KiB를 없애고 세 경로의 FCP를 줄였으며, 수식 글의 5초대 LCP도 없앴다. 수식 글꼴 자체의 용량은 줄이지 않았다. 24회 관찰 구간의 CLS는 모두 0이었지만, 운영체제별 글자 모양이 이전과 같다는 뜻은 아니다. [실제 수정 후 측정 원자료](https://github.com/yceffort/blog/blob/09afeb98/apps/blog/tests/performance/series-overall/font-removal-results.json)에 개별 실행과 LCP 대상을 함께 남겼다.
...
xychart-beta horizontal
    ...
    x-axis ["홈 전", "홈 후", "코드 전", "코드 후", "수식 전", "수식 후"]
    y-axis "KiB" 0 --> 1600
    bar [765.9, 719.8, 1096.3, 1070.3, 787.6, 1073.2]
```
...
xychart-beta horizontal
    ...
    x-axis ["홈 전", "홈 후", "코드 전", "코드 후", "수식 전", "수식 후"]
    y-axis "KiB" 0 --> 700
    bar [232.3, 168.3, 232.3, 168.3, 232.3, 502.3]
```
...
xychart-beta horizontal
    title "블로그 빌드 시간"
    x-axis ["변경 전 (167.00초)", "변경 후 (20.92초)"]
    y-axis "초" 0 --> 200
    bar [167.0, 20.92]
```
...
5.6초라는 수치의 뜻도 바로잡았다. 별도로 화면을 확인한 실행에서는 약 2초에 본문 첫 문단의 시작이 이미 보였다. 이후 글꼴 로드가 끝나면서 그 문단이 LCP로 새로 기록됐다. 글꼴 전송 시점을 바꾸면 늦은 후보도 달라졌지만, 제목이나 배너가 최종 후보가 되는 실행도 있었다. 다음 변경의 효과를 확인할 때도 LCP 숫자만 비교하지 않고 실제로 보인 화면과 선택된 요소를 함께 보려 한다.
