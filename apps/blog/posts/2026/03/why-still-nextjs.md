...
왜 그런가? **Next.js가 지금도 기본값인 이유는 런타임이 빨라서가 아니라, 바꾸는 비용이 너무 높기 때문이다.** 생태계, 플랫폼, 채용 시장, 학습 자산 — Vercel이 쌓아 올린 것들이 전환 비용을 만들고, 그 전환 비용이 Next.js를 지탱한다. 그리고 같은 구조가 Next.js의 강점과 약점을 동시에 만들어낸다. 빛과 어둠이 다른 곳에서 오는 것이 아니라, 같은 설계에서 나온다.
...
| Stack Overflow 질문[^2] | 60,000+ | Nuxt ~15,000 | 학습 자산 축적의 지표   |
...
이 구조가 왜 만들어졌는지를 이해하려면, Vercel의 인센티브를 봐야 한다. Vercel은 2024년 Series E 기준 누적 $5.6B 이상의 기업 가치를 인정받았고, 그에 상응하는 투자금 회수 압력이 존재한다. 수익은 플랫폼 사용량에 연동되므로, Next.js의 기능이 Vercel 인프라를 더 많이 소비하는 방향으로 설계되는 것은 음모가 아니라 인센티브의 자연스러운 귀결이다 — ISR, Edge Functions, 세그먼트별 프리페치 모두 서버리스 호출 수를 늘리는 기능이다. 이것은 Vercel이 "나쁘다"는 뜻이 아니라, 오픈소스 프레임워크의 개발 우선순위가 상장 전 기업의 매출 목표와 정렬되어 있는 구조적 현실이다.
...
- **대규모 채용 조직**: 채용 시장의 관성 때문에 마지막까지 Next.js를 기본값으로 유지할 가능성이 높다.
...
여기에 **Next.js 내부의 분열**도 있다. App Router가 출시된 지 3년이 지났지만, 상당수 프로덕션 코드베이스는 여전히 Pages Router에 머물러 있다. App Router로의 마이그레이션 자체가 고통스럽기 때문이다 — 데이터 페칭 패턴의 전면 재작성(`getServerSideProps` → Server Components), 레이아웃 모델 변경, 캐싱 전략의 근본적 차이. Next.js를 쓰고 있는 팀 안에서도 이미 Pages Router에 남는 쪽과 App Router로 이동하는 쪽으로 경험이 갈라지고 있으며, 이것은 "Next.js 안에서 이미 이탈이 진행 중"이라는 의미다. 프레임워크를 떠나기도 전에 프레임워크 안에서의 마이그레이션에 지치고 있는 것이다.
...
기술적으로 "Next.js가 아니면 안 되는" 시나리오는 점점 줄고 있다. RSC가 반드시 필요한 경우가 아니라면, TanStack Start나 React Router v7이 같은 React 19 위에서 더 가벼운 런타임을 제공한다. 물론 런타임 구조만으로 프레임워크를 선택하지는 않는다. 문서, 채용 시장, 서드파티 통합, 트러블슈팅 자산까지 포함하면 Next.js의 우위는 여전히 크다.
...
거친 추정을 하자면, 현재 추세로는 이 세 가지가 동시에 충족되기까지 최소 3~5년이 걸릴 가능성이 높다. 채용 시장의 관성은 기술 트렌드보다 느리게 움직이고, 서드파티 생태계가 재편되려면 대안 프레임워크의 시장 점유율이 임계치(대략 현재의 10배 이상)를 넘어야 한다. jQuery → React 전환이 본격화되기까지 약 4~5년(2013년 React 출시 → 2017~2018년 채용 시장 주류화)이 걸렸던 것을 참고하면, Next.js 이후의 전환도 비슷한 시간 규모를 요구할 것이다.
...
Stack Overflow의 60,000+ 질문, GitHub의 수백만 개 리포지토리, 수만 개의 블로그 포스트와 튜토리얼. Next.js는 React 메타 프레임워크 중 가장 많은 텍스트 데이터를 생성해왔고, 이 데이터가 LLM의 학습 코퍼스에 포함되어 있다. 물론 LLM 모델의 학습 데이터 구성은 공개되지 않으므로, "데이터가 많으면 생성 품질이 높다"는 추론이지 검증된 사실은 아니다. 하지만 이 추론을 뒷받침하는 정황은 있다.

실무에서 이것은 직접적인 영향을 미친다. AI에게 프레임워크를 지정하지 않고 "이커머스 상품 페이지를 만들어줘"라고 요청하면, 높은 확률로 Next.js App Router + Server Components 코드가 나온다. TanStack Start 코드를 생성하려면 명시적으로 지정해야 하고, 그래도 품질이 떨어지는 경우가 많다 — 이것은 필자가 Claude 3.5 Sonnet, GPT-4o, Copilot에서 동일한 프롬프트로 반복 테스트한 경험적 관찰이며, 체계적 벤치마크는 아니다. 프레임워크의 AI 친화도가 개발 생산성에 직결되는 시대에, **AI는 Next.js의 경로 의존성을 강화하는 새로운 수확 체증(increasing returns) 메커니즘이다.**
...
프레임워크를 계속 쓰는 것보다 위험한 것은, 왜 계속 쓰는지 점검하지 않는 것이다. 이 시리즈가 그 점검에 조금이라도 도움이 되었길 바란다.
...
[^5]: Igor Minar, ["Claude has written the majority of the code in this project"](https://x.com/AustinPlays0/status/1894504792392745365), vinext 공개 당시.
