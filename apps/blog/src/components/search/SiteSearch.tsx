...
import * as searchInputStyles from '@/components/search/search-input.styles'
...
export default function SiteSearch() {
  ...
  return (
    <>
      ...
      {open && (
        <div
          className={`search-overlay ${searchStyles.search_overlay}`}
          role="presentation"
          onClick={(e) => {
        ...
        >
          <dialog
            open
            className={`search-panel ${searchStyles.search_panel}`}
            aria-modal="true"
            aria-label={locale === 'en' ? 'Search posts' : '글 검색'}
          >
            <div
              className={`search-input-row ${searchStyles.search_input_row}`}
            >
              <svg
                width="18"
                height="18"
              ...
              <input
                ref={inputRef}
                type="search"
                className={`search-input ${searchStyles.search_input} ${searchInputStyles.element_input}`}
              ...
              <button
                type="button"
                className={`search-esc ${searchStyles.search_esc}`}
                onClick={handleClose}
            ...
            <div className={`search-results ${searchStyles.search_results}`}>
              {loading && (
                <p className={`search-hint ${searchStyles.search_hint}`}>
              ...
              {results.map((item) => (
                <Link
                  ...
                  className={`search-result ${searchStyles.search_result}`}
                  onClick={handleClose}
                  prefetch={false}
                >
                  <span
                    className={`search-result-title ${searchStyles.search_result_title}`}
                  >
                    {highlight(item.title, item.terms)}
                  </span>
                  {item.snippet && (
                    <span
                      className={`search-result-desc ${searchStyles.search_result_desc}`}
                    >
                      {highlight(item.snippet, item.terms)}
                    </span>
                  )}
                  <span
                    className={`search-result-meta ${searchStyles.search_result_meta}`}
                  >
                    {item.date}
                    {item.tags.length > 0 &&
                      ` · ${item.tags
            ...
            {locale !== 'en' && (
              <div className={`search-foot ${searchStyles.search_foot}`}>
                <Link
                  href="/archive"
                  className={`search-archive-link ${searchStyles.search_archive_link}`}
                  onClick={handleClose}
                >
                  연도별로 둘러보기 →
                </Link>
