...
pub fn resolve(tokenizer: &mut Tokenizer) -> Option<Subresult> {
    ...
    let mut open_sequence_exit: Option<usize> = None;

    while index < tokenizer.events.len() {
        let name = tokenizer.events[index].name.clone();
        let kind = tokenizer.events[index].kind.clone();

        if name == Name::HeadingAtx {
            if kind == Kind::Enter {
                heading_inside = true;
                open_sequence_exit = None;
            } else {
                if let Some(start) = data_start {
                    // If `start` is some, `end` is too.
                    let end = data_end.unwrap();

                    // 패치: micromark 처럼 본문은 여는 시퀀스(와 뒤 공백) 바로 다음부터다.
                    // `### #1. foo` 의 안쪽 `#` 시퀀스가 첫 Data 앞에 있으면 본문에 넣는다:
                    // 그 사이 이벤트를 지우고 Data 의 시작점을 앞으로 당긴다.
                    let mut text_start = start;
                    if let Some(open_exit) = open_sequence_exit {
                        let mut content_start = open_exit + 1;
                        if tokenizer.events[content_start].name == Name::SpaceOrTab {
                            content_start += 2;
                        }
                        if content_start < start {
                            let point = tokenizer.events[content_start].point.clone();
                            tokenizer.events[start].point = point;
        ...
        } else if heading_inside && name == Name::HeadingAtxSequence {
            if kind == Kind::Exit && open_sequence_exit.is_none() {
                open_sequence_exit = Some(index);
            }
        } else if heading_inside && name == Name::Data {
            if kind == Kind::Enter {
