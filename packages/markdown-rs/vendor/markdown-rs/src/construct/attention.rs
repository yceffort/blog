    ...
    while index < tokenizer.events.len() {
        ...
        if enter.name == Name::AttentionSequence {
            if enter.kind == Kind::Enter {
                ...
                let gfm_strikethrough = tokenizer.parse_state.options.constructs.gfm_strikethrough;
                let before_is_marker = matches!(before_char, Some('*' | '_'))
                    || (gfm_strikethrough && before_char == Some('~'));
                let after_is_marker = matches!(after_char, Some('*' | '_'))
                    || (gfm_strikethrough && after_char == Some('~'));
                // 패치: `*`/`_` 는 remark-cjk-friendly 규칙으로 판정한다.
                // 취소선(`~`)은 원래 규칙 그대로 (remark-cjk-friendly 도 손대지 않는다).
                let (open, close) = if marker == b'~' {
                    ...
                    let two_before_char = before_char.and_then(|c| {
                        char_before_index(tokenizer.parse_state.bytes, enter.point.index - c.len_utf8())
                    });
                    cjk::flanking(
                        marker,
                        before_char,
